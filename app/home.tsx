...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios
      .get(`http://10.130.218.95:3000/hotels/city/${cityData.name}`)
      .then((response) => {
        if (response.data && response.data.hotels) {
          setHotels(response.data.hotels);
        } else {
  ...
  return (
    <View style={{ marginBottom: 400 }}>
      <FlatList
        ...
        renderItem={({ item }) => (
          ...
          >
            <Image 
              source={{ 
                uri: item.url || 'https://via.placeholder.com/400x300?text=Hotel+Image'
              }} 
              style={styles.hotelImageSingle}
            />
            <View style={styles.hotelInfoContainer}>
              <View style={styles.hotelTextContainer}>
                <Text style={styles.hotelNameSingle}>{item.hotel_name}</Text>
                <Text style={styles.hotelClassBadge}>{item.hotel_class} ★</Text>
              </View>
            </View>
          </TouchableOpacity>
        )}
      ...
      return (
        <View style={{ marginBottom: 400 }}>
          <FlatList
            key="foodList" // Adding a key prop
            data={restaurants}
            keyExtractor={(item) => item.place_id}
            renderItem={({ item }) => (
              <View style={styles.cityCard}>
      ...
      return (
        <View style={{ marginBottom: 400 }}>
          <FlatList
            key="carRentalsList" // Adding a key prop
            data={carRentals}
            keyExtractor={(item) => item._id}
            renderItem={({ item }) => (
              <View style={styles.card}>
