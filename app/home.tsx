...
export default function HomeScreen() {
  ...
  const handleSearch = async (query: string) => {
    ...
    if (query.length >= 3) {
      setSearchLoading(true);
      try {
        const response = await axios.get(`${API_BASE_URL}/api/search`, { params: { query } });
        if (response.data.status === "OK" && response.data.results.length > 0) {
          setSearchResults(response.data.results);
        } else {
          setSearchResults([]);
  ...
  // Hotel card button handlers (same as in CityScreen.tsx)
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={hotelStyles.card}>
              ...
              <View style={hotelStyles.buttonsContainer}>
                <TouchableOpacity style={hotelStyles.button} onPress={() => handleNavigate(item.hotel_name)}>
                  <Text style={hotelStyles.buttonText}>Navigate</Text>
                </TouchableOpacity>
                <TouchableOpacity style={hotelStyles.button} onPress={() => handleCheckReviews(item.hotel_name)}>
                  <Text style={hotelStyles.buttonText}>Check Reviews</Text>
                </TouchableOpacity>
                <TouchableOpacity style={hotelStyles.button} onPress={() => handleMakeReservation(item.hotel_name)}>
